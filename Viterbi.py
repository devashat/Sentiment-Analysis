...
def decode(input_length, tagset, score):
    ...
    """
    scores = np.zeros((len(tagset), input_length)) # Score of most likely path so far
    v_matrix = np.zeros((len(tagset), input_length)) # Most likely path so far
    # initialize matrices to 0
    for i, tag in enumerate(tagset):
        scores[i][1] = 0 
        v_matrix[i][1] = 0
    tagset2 = ["<START>"] + tagset # Used in forloop for accounting for "<START>" tag
    for j in range(1, input_length):
        for i, tag in enumerate(tagset):
            # First word in input, only possible tag is <START>
            if j == 1:
                tagset2 = ["<START>"]
            else:
                tagset2 = tagset
            # Last Word in index, only possible next word is <STOP>
            if j == input_length-1:
                tagset1 = ["<STOP>"]
                i2 = 0
            else:
            ...
            scores[i][j] = max(scores[k, j-1]+score(tagset1[i2], tagset2[k], j) for k in range(len(tagset2))) #Save maximum score for tag i at input position j
            v_matrix[i][j] = np.argmax(np.asarray(list(scores[k, j-1]+score(tagset1[i2], tagset2[k], j) for k in range(len(tagset2))))) # Save maximum scoring tag for tag i at input position j
    
    highest_index = [0] + ([0] * (input_length-1)) #List of tag indexs (from tagset) with the highest score
    best_path = ["<START>"] + ([0] * (input_length-2)) + ["<STOP>"] # List of tags corresponding to the highest scoring path of tags 
    last_tag = np.argmax(np.asarray(scores[k, input_length-1] for k in range(len(tagset)))) # Calculate index of best scoring last tag (not including <STOP>)
    highest_index[input_length-1] = last_tag # Store last tag in list of last tags
    best_path[input_length-2] = tagset[last_tag] # Add last tag to path being stored

    #Working backwards, find all other previous tags corresponding to the best path
    for j in range(input_length-1, 1, -1):
         highest_index[j-1] = np.int64(v_matrix[highest_index[j], j])
         best_path[j-1] = tagset[highest_index[j-1]] 
    return best_path
